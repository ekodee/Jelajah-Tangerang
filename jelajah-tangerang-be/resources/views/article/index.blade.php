                            ...
                            <tr>
                                ...
                                <td class="align-middle">
                                    <div class="d-flex align-items-center">
                                        @php
                                            $imageSrc = Str::startsWith($article->thumbnail, 'http')
                                                ? $article->thumbnail
                                                : asset('storage/' . $article->thumbnail);
                                        @endphp

                                        <img src="{{ $imageSrc }}" class="rounded me-2" width="48" height="48"
                                ...
                                <td class="align-middle">
                                    <span class="badge bg-info text-white">{{ $article->category->name ?? 'Umum' }}</span>
                                </td>
                                <td class="align-middle">
                                    <span
                                        class="badge bg-light text-dark border">{{ $article->user->name ?? 'Unknown' }}</span>
                                </td>
                                <td class="align-middle">
                                ...
                                <td class="table-action text-end align-middle">
                                    <a href="{{ route('artikel.edit', $article->id) }}" class="text-info me-2"
                                        title="Edit">
                                        <i class="align-middle" data-feather="edit-2"></i>
                                    </a>
                                    <form action="{{ route('artikel.destroy', $article->id) }}" method="POST"
                                        class="d-inline" onsubmit="return confirm('Yakin hapus artikel ini?')">
                                        @csrf
                                        @method('DELETE')
